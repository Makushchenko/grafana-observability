...
- [The New Stack (TNS) Observability App](#the-new-stack-tns-observability-app)
  ...
  - [Install TNS demo (running MLT stack locally)](#install-tns-demo-running-mlt-stack-locally)
  - [Install TNS demo app into an existing K8s cluster (`app-only` option)](#install-tns-demo-app-into-an-existing-k8s-cluster-app-only-option)
  - [Install TNS demo app connected to Grafana Cloud (`grafana-cloud` option)](#install-tns-demo-app-connected-to-grafana-cloud-grafana-cloud-option)
...
Since much of the program is written in Go, you should have Go installed on your local machine. Click [here](https://go.dev/doc/install) for installation instructions.
...
- For Ubuntu 20.04 and lower, use [k3d v3.2.0](https://github.com/rancher/k3d/releases/tag/v3.2.0).
- For Ubuntu 21.10 and higher (which has cgroups v2 enabled by default, and k8s fails to start thinking cgroups are not available), use [k3d v5.0.0](https://github.com/rancher/k3d/releases/tag/v5.0.0)
...
6. Access TNS using the URL [http://localhost:8080/](http://localhost:8080).
...
4. Click on a data point to see the exemplar data in an overlay, then click "Query with Tempo" in the exemplar overlay. (If no data points appear, make sure exemplars are enabled in the query options.)
5. In the trace viewer, click the log icon of one of the span lines to view log details.
